...
const CostCalculator = () => {
    ...
    const [userData, setUserData] = useState({
        'entry.827641169': '',  // Name
        'entry.395162595': '',  // Email
        'entry.109147595': '',  // Contact
        'entry.174549387': '',  // Address
        'entry.924826971': estimatedCost,  // Estimated cost
        'entry.1083149044': formData.cleaningType,  // Service type
        'entry.2119954005': formData.addOns.join(', '),  // Add-ons
    });
    ...
    const handleFormSubmit = (e) => {
        ...
        const googleFormBaseURL = `https://docs.google.com/forms/d/e/1FAIpQLSduWJj7w5RKWRpAUNYdJq97sRgNYu8yqVyKDuqc1EMhRJJihw/viewform?usp=pp_url`;

        const prefilledURL = `${googleFormBaseURL}&entry.827641169=${encodeURIComponent(userData['entry.827641169'])}&entry.395162595=${encodeURIComponent(userData['entry.395162595'])}&entry.109147595=${encodeURIComponent(userData['entry.109147595'])}&entry.174549387=${encodeURIComponent(userData['entry.174549387'])}&entry.924826971=${encodeURIComponent(estimatedCost)}&entry.1083149044=${encodeURIComponent(formData.cleaningType)}&entry.2119954005=${encodeURIComponent(formData.addOns.join(', '))}`;

        window.open(prefilledURL, "_blank");
    };

    // https://docs.google.com/forms/d/e/1FAIpQLSduWJj7w5RKWRpAUNYdJq97sRgNYu8yqVyKDuqc1EMhRJJihw/viewform?usp=pp_url&entry.827641169=name&entry.395162595=email&entry.109147595=contact&entry.174549387=address&entry.924826971=estimatedcost&entry.1083149044=service&entry.2119954005=addons
...

            {
                ...
                <>
                    <div className='flex flex-col gap-4 md:w-1/2 mt-10'>
                        ...
                        <p>
                            Click the button below to get a quote for the estimated cost. Your details will be submitted to our team and we will get back to you with a quote.
                        </p>

                        <a target='_blank' href={BOOKING_FORM_URL} className='bg-[#6B6869] text-lg text-white font-medium py-2 px-10 w-max rounded-lg'>
                            Book Now
                        </a>
                    ...
                    {/* <form method='POST' onSubmit={handleFormSubmit} className='flex flex-col gap-4 md:w-1/2 mt-10'>
                        ...
                        <input
                            type='text'
                            name='entry.827641169'
                            value={userData['entry.827641169']}
                        ...
                        <input
                            type='text'
                            name='entry.109147595'
                            value={userData['entry.109147595']}
                            onChange={userDataChange}
                        ...
                        <input
                            type='email'
                            name='entry.395162595'
                            value={userData['entry.395162595']}
                            onChange={userDataChange}
                            placeholder='Email'
                            className='outline outline-2 outline-slate-300 focus:border-none focus:outline-blue-900 py-2 px-5 rounded-lg duration-300'
                        />
                        ...
                        <input
                            type='text'
                            name='entry.924826971'
                            value={userData['entry.924826971']}
                            onChange={userDataChange}
                            placeholder='Address'
                            className='outline outline-2 outline-slate-300 focus:border-none focus:outline-blue-900 py-2 px-5 rounded-lg duration-300'
                        />
